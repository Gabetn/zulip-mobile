...
import * as eg from '../../__tests__/exampleData'; // $FlowFixMe Make flow understand about mocking
...
describe('queueMarkAsRead', () => {
  ...
  test('should call messagesFlags, if calls to queueMarkAsRead are 2s apart', () => {
    const start = Date.now(); // $FlowFixMe Make flow understand about mocking

    Date.now = jest.fn().mockReturnValue(start);
    queueMarkAsRead(eg.selfAuth, [13, 14, 15]); // $FlowFixMe Make flow understand about mocking
  ...
  test('should set timeout for time remaining for next API call to clear queue', () => {
    ...
    Date.now = jest.fn().mockReturnValue(start);
    queueMarkAsRead(eg.selfAuth, [1, 2, 3]); // $FlowFixMe Make flow understand about mocking

    Date.now = jest.fn().mockReturnValue(start + 1900);
    queueMarkAsRead(eg.selfAuth, [4, 5, 6]); // $FlowFixMe Make flow understand about mocking
